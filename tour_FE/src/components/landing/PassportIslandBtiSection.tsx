...
function formatIslandBtiTestDate(testedAt: string): string | null {
...
export function PassportIslandBtiSection({ variant = "default" }: PassportIslandBtiSectionProps) {
  ...
  const { currentIslandBtiResult } = useProfileCharacter();
  const isPassport = variant === "passport";

  if (!currentIslandBtiResult) {
    return (
      <section
        className={[
  ...
  const profile = getIslandBtiResult(currentIslandBtiResult.code);
  if (!profile) {
    return (
      <section
        className={[
  ...
  const testedAtLabel = formatIslandBtiTestDate(currentIslandBtiResult.testedAt);
  ...
  const handleViewDetail = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    navigate("/island-bti/result", {
      state: {
        result: currentIslandBtiResult.code,
        scores: currentIslandBtiResult.scores,
      },
    });
  };
