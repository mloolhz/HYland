...
function formatTestDate(testedAt: string): string | null {
...
export function PassportBtiCard({ allowDemoPreview = false }: PassportBtiCardProps) {
  ...
  const effectiveResult =
    currentIslandBtiResult ?? (allowDemoPreview ? DEMO_PASSPORT_ISLAND_BTI : null);
  ...
  const testedAtLabel = formatTestDate(effectiveResult.testedAt);
  const themeStyle = { "--bti-theme": profile.themeColor } as CSSProperties;
  const isDemoOnly = !currentIslandBtiResult && allowDemoPreview;
