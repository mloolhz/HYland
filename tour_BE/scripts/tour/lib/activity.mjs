...
const RULES = [
  ...
  // ── 해상레저 ──
  ["요트", "SEA", /요트|마리나|선셋요팅/],
  ["유람선", "SEA", /유람선|여객선|크루즈|선착장|나룻배/],
  ["패들보드", "SEA", /패들|sup|보트|제트/],

  // ── 체험 (시설형) ──
  ["골프", "EXPERIENCE", /골프|컨트리클럽|golf|gc|cc(?![a-z])/],
  ["수련단체활동", "EXPERIENCE", /수련원|연수원|교육원|학습관|평생학습/],
...
export function resolveActivity(name, extra = "") {
  ...
  return { activity: "기타", category: "EXPERIENCE", matched: null };
}
