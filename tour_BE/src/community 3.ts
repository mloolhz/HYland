...
import { analyzeWithLexicon } from "./services/community-analysis";
  ...
  try {
    ...
    res.json({
      ...
      posts: rows.map((p) => ({
        ...
        mentionedActivities: Array.isArray(p.mentionedActivities) ? p.mentionedActivities : undefined,
        bestMonths: Array.isArray(p.bestMonths) ? p.bestMonths : undefined,
        companionFit: Array.isArray(p.companionFit) ? p.companionFit : undefined,
        cautions: Array.isArray(p.cautions) ? p.cautions : undefined,
      })),
    });
  } catch (err) {
...
router.post("/posts", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const analysis = analyzeWithLexicon(title.trim(), content.trim());
